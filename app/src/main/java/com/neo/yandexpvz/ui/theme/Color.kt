package com.neo.yandexpvz.ui.theme

import androidx.compose.material.Colors
import androidx.compose.ui.graphics.Color

val Purple200 = Color(0xFFBB86FC)
val Purple500 = Color(0xFF6200EE)
val Purple700 = Color(0xFF3700B3)
val Teal200 = Color(0xFF03DAC5)


//custom color
val Colors.OrangeColor get() = Color(0xFFFF7643);
val Colors.OrangeDarkColor get() = Color(0xFFF57C00);
val Colors.OrangeMediumColor get() = Color(0xFFFACB7C);
val Colors.Yellow get() = Color(0xFFFFCC00)
val Colors.OrangeLightColor get() = Color(0xFFFFF5EF);
val Colors.SecondaryColor get() = Color(0xFF979797)
val Colors.LightGray get() = Color(0xFFFAFAFA)
val Colors.TextColor get() = Color(0xFF757575)
val Colors.YellowText get() = Color(0xFFF3DE27)
val Colors.BlueText get() = Color(0xFF06187E)
val Colors.WarningColor get() = Color(0xFFF1382B);
val Colors.WhiteColor get() = Color(0xFFF3F3F3);